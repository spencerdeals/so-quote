// index.js — so-quote backend (ScrapingBee primary, fallback optional)
...
function normalizeOne(obj={}, fallbackUrl="") {
  ...
  return { title, image, price, variant, url: obj.url || fallbackUrl };
}

function tryParseHtml(html="", url="") {
  const out = { url };
  const t = html.match(/og:title\"?['"]?\s+content=['"]([^'"]+)/i);
  const i = html.match(/og:image\"?['"]?\s+content=['"]([^'"]+)/i);
  if (t) out.title = t[1];
  if (i) out.image = i[1];
  return out;
}
...
async function scrapeWithBee(targetUrl) {
  ...
  const endpoint = `https://app.scrapingbee.com/api/v1/?api_key=${beeKey()}&url=${encodeURIComponent(targetUrl)}&render_js=true&block_resources=true&premium_proxy=true`;
